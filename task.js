...
//Buat array baru dengan data - data berikut
let idTransaction = [],
    storeLocations = [],
    userEmails = [],
    purchaseMethods = [],
...
data.forEach(function (d) {
    idTransaction.push(d.id),
        storeLocations.push(d.storeLocation),
        userEmails.push(d.customer.email),
        purchaseMethods.push(d.purchaseMethod)
        items.push(d.items)
})

// Kelompokkan data data transaksi dalam bentuk array pada sebuah object, berdasarkan lokasi pembelian
...
function getAllPayMethods(){
    dataMap.forEach(function(d){
        payMethods.push(d.purchaseMethod)
    })
...
// console.log(removeDuplicates(payMethods))
let ObjData = {}
pay = removeDuplicates(payMethods)
for (let i=0;i<pay.length;i++){
    const purchaseMethode = dataMap.filter(function (d) {
        purchaseMethodes = d.purchaseMethod == pay[i]
        return purchaseMethodes
})
ObjData[`${pay[i]}`] =  purchaseMethode
}
console.log(ObjData.Phone)
console.log(ObjData.Online)
console.log(ObjData[`In store`])

// console.log(dataMap)
// let ObjData = {}

// function findAllByPurchaseMethod(data, key) {
//     const purchaseMethode = data.filter(function (d) {
//         purchaseMethodes = d.purchaseMethod == key
//         return purchaseMethodes
//     })
//     if (key == "Online"){
//         ObjData.PurchaseOnline = purchaseMethode
//     }else if(key == "In store"){
//         ObjData.PurchaseInStore = purchaseMethode
//     }else if(key == "Phone"){
//         ObjData.PurchaseInPhone = purchaseMethode
//     }
// }
// findAllByPurchaseMethod(dataMap, "In store")
// findAllByPurchaseMethod(dataMap, "Phone")
// findAllByPurchaseMethod(dataMap, "Online")
// console.log(ObjData)



//Buat array yang menyimpan keuntungan yang didapat dari setiap transaksi

const revenue = dataMap.map(function(d){
    revenues = "$"+(d.items.price.$numberDecimal * d.items.quantity.$numberInt).toFixed(2)
    return revenues 
})

console.log(revenue)

//Buat array berdasarkan script pada link diatas yang menyimpan jumlah barang yang dibeli pada setiap transaksi
const quantity = dataMap.map(function(d){
    quantities = d.items.quantity.$numberInt
    return quantities
})
console.log(quantity)

//Buat array berdasarkan script pada link diatas yang menyimpan rating setiap toko dari satisfaction customer mereka
const satisfaction  = data.map(function(d){

   rateSatisfactions = d.customer.satisfaction.$numberInt
    return rateSatisfactions
})
...
LocStores = []
ObjDataLocations = {}
function getAllStoreLoc(){
    dataMap.forEach(function(d){
        LocStores.push(d.storeLocation)
    })
    
}
getAllStoreLoc()
console.log(removeDuplicates(LocStores))
Store = removeDuplicates(LocStores)
for (let i=0;i<Store.length;i++){
    const locationStore = dataMap.filter(function (d) {
        locationStores = d.storeLocation == Store[i]
        return locationStores
})
const rev = locationStore.map(function(d){
        revenueLoc = "$" +(d.items.price.$numberDecimal * d.items.quantity.$numberInt).toFixed(2)
        return revenueLoc
    })
ObjDataLocations[`${Store[i]}`] =  rev
}
console.log(ObjDataLocations)
